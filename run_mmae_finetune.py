...
def get_args():
    ...
    parser.add_argument('--linprobe_n', default=[100, 1000, 10000], type=int, nargs='+',
    ...
    # Model parameters
    parser.add_argument('--model', default='multivit_small', type=str, metavar='MODEL',
    ...
    parser.add_argument('--blr', type=float, default=3e-3, metavar='LR',
    ...
    parser.add_argument('--min_lr', type=float, default=6e-5, metavar='LR',
...
def main(args):
    ...
    for n_samples in args.linprobe_n:
        ...
        for fold, (train_ids, val_ids) in enumerate(kfold.split(dataset_train, y)):
            ...
            df = df.groupby('y', group_keys=False).apply(lambda x: x.sample(min(len(x), num_samples_per_label), random_state=seed))
...
if __name__ == '__main__':
    ...
    train_images_path = os.path.join(PROCESSED_BASE_DIR, 'pyramid', f"{args.dataset}", f"Test", 'mae_patches.csv')
