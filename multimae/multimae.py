...
class MultiMAE(nn.Module):
    """MultiMAE_olde: Multi-task Multi-modal Masked Autoencoder
    ...
    def generate_random_masks(self,
        ...
        :param mask_strategy: which strategy to use in rearanging mask, random or encode in only 1 modal

        """
        B = list(input_tokens.values())[0].shape[0]
        N = list(input_tokens.values())[0].shape[1]

        device = list(input_tokens.values())[0].device

        alphas = [alphas[0]] * len(input_tokens) if len(alphas) == 1 else alphas
        ...
        elif mask_strategy == 'one':
            bins = torch.arange(0, len(num_tokens_per_task)) * N
            # assumes that eash modality is patched identical in terms of number of patches and spatial arrangement
            noise = torch.rand(B, N, device=device)  # noise in [0, 1]
            # sort noise for each sample
            ids_shuffle = torch.argsort(noise, dim=1)  # ascend: small is keep, large is remove

            cat_ids_shuffle = []
            for batch, len_keep in zip(range(B), samples_per_task):
                # print(len_keep)
                row = ids_shuffle[batch, :]
                unique = row[:sum(len_keep)]
                cat_ids = torch.cat([row + bn for bn in bins], dim=0)
                # scatter
                noise_index = torch.argsort(torch.rand(N * len(num_tokens_per_task)), dim=0)
                cat_ids = torch.gather(cat_ids, dim=0, index=noise_index)

                taken = 0
                last_idx = len(len_keep) - 1
                __unique = []
                for idx, task_len in enumerate(len_keep):
                    if idx == last_idx:
                        # this solves the issue of incomplete sum of samples_per_task's len_keep: a
                        # dirichelet dist issue mitigated in mmae using another noise infusion to shuffle
                        # [0, 5, 2]
                        __unique.append(unique[taken:] + (N * idx))
                    else:
                        __unique.append(unique[taken: taken + task_len] + (N * idx))
                    taken += task_len
                __unique = torch.cat(__unique, dim=0)

                indcs = torch.cat([(cat_ids == i).nonzero() for i in __unique], 1)
                mask = torch.ones(cat_ids.numel(), dtype=torch.bool)
                mask[indcs] = False
                cat_ids = torch.cat((__unique, cat_ids[mask]), dim=0)
                cat_ids_shuffle.append(cat_ids.unsqueeze(0))

            cat_ids_shuffle = torch.cat(cat_ids_shuffle, dim=0)
            ids_restore = torch.argsort(cat_ids_shuffle, dim=1)
            ids_keep = cat_ids_shuffle[:, :num_encoded_tokens]

            # Update binary mask to adjust for task rounding
            mask_all = torch.ones_like(cat_ids_shuffle)
            mask_all[:, :num_encoded_tokens] = 0
            # Unshuffle to get the binary mask
            mask_all = torch.gather(mask_all, dim=1, index=ids_restore)
            # Split to get task masks
    ...
    def forward(self, 
        ...
        if 'rgb' in x:
            B, C, H, W = x['rgb'].shape
        elif 'semseg' in x: # not changing for now
