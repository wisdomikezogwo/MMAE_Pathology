...
from multimae.criterion import MaskedL1Loss, MaskedMSELoss
...
from utils.dataset_histo_new import generate_lim_mae_csv, AeSimEmbedDatasetWrapper
...
def get_args():
    ...
    parser = argparse.ArgumentParser('MultiMAE_olde pre-training script', add_help=False)

    parser.add_argument('-d', '--dataset', default='NCK', choices=['uwsf', 'NCK'],
                        help='dataset network must be pretrained on',
                        type=str)

    parser.add_argument('-tm', '--train_mode', default='pretrain', choices=['pretrain', 'downstream'],
                        help='train mode for dataset',
                        type=str)

    parser.add_argument('-sc', '--scale', default=20.0, choices=[1.25, 5.0, 10.0, 20.0], type=float)

    parser.add_argument('-no_vs', '--no_visualize', action='store_true',
                        help='whether or not to send reconstructed images to tensorboard to be visualized'
                             'if no_t is True then this will not matter as nothing will be sent to tb at all')
    ...
    parser.add_argument('--linprobe_n', default=None, type=int,
                        help='number for patches to use for training linear classifier:Default None, since pretraining')

    parser.add_argument('--num_images_per_patient', default=1200, type=int,
                        help='size of paches per patient to use in making mae_patches.csv (default: %(default)s)')
    #############################################################################################

    # MASKING STRATEGY #####################

    parser.add_argument('-ms', '--mask_strategy', default='', choices=['', 'one'],
                        help='mask_strategy', type=str)
    parser.add_argument('--alphas', default=[1.0], type=float, nargs='+',
    ...
    # Task parameters
    parser.add_argument('--in_domains', default='rgb-him-eim', type=str,
    ...
    # Model parameters
    parser.add_argument('--model', default='pretrain_multimae_small', type=str, metavar='MODEL',
...
def main(args):
    ...
    if len(args.in_domains) > 1:
        args.pre_extracted_h_e = True
    else:
        args.pre_extracted_h_e = False
    ...
    # Get dataset
    pathology_dataset_class = AeSimEmbedDatasetWrapper(args)
    ...
    if log_writer is not None:
        visualize_(model=model, data_loader=data_loader_train, device=device,
                   epoch=0, log_writer=log_writer, num_encoded_tokens=args.num_encoded_tokens,
                   in_domains=args.in_domains, out_domains=args.out_domains, alphas=args.alphas,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.output_dir:
            if (epoch + 1) % args.save_ckpt_freq == 0 or epoch + 1 == args.epochs:
                ...
                if log_writer is not None:
                    visualize_(model=model, data_loader=data_loader_train, device=device,
                               epoch=epoch, log_writer=log_writer, num_encoded_tokens=args.num_encoded_tokens,
                               in_domains=args.in_domains, out_domains=args.out_domains, alphas=args.alphas,
                               sample_tasks_uniformly=args.sample_tasks_uniformly,
                               standardize_depth=args.standardize_depth,
                               fp32_output_adapters=args.fp32_output_adapters.split('-'),
...
def train_one_epoch(model: torch.nn.Module, data_loader: Iterable, tasks_loss_fn: Dict[str, torch.nn.Module],
                    ...
                    alphas: List[float] = [1.0], sample_tasks_uniformly: bool = False, standardize_depth: bool = True,
    ...
    for step, (x, _) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        if standardize_depth and 'him' in tasks_dict:
            ...
            tasks_dict['him'] = (tasks_dict['him'] - trunc_depth.mean(dim=1)[:,None,None,None]) / torch.sqrt(trunc_depth.var(dim=1)[:,None,None,None] + 1e-6)
        ...
        if log_writer is not None:
            ...
            for things_to_log in [log_this, task_loss_values, weighted_task_loss_values]:
                for name, value in things_to_log.items():
                    log_writer.add_scalar(f'{name}', value, epoch_1000x)
...
if __name__ == '__main__':
    ...
    train_images_path = os.path.join(PROCESSED_BASE_DIR, 'pyramid', f"{args.dataset}", f"Train", 'mae_patches.csv')
